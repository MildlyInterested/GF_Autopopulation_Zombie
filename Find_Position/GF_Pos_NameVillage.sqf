...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position NameVillage Found %1",GF_APZ_Safe_distance_Check_Ok];
	};	

	if (GF_APZ_Diag_Log_info) then {
	diag_log format ["Position NameVillage Found %1",GF_APZ_Safe_distance_Check_Ok];
	};		
