...
GF_APZ_randomPos_ShoreLine_Shore = {
...
if (
	...
	systemchat format ["Position ShoreLine_Shore Found %1",GF_APZ_Safe_distance_Check_Ok];
	};	

	if (GF_APZ_Diag_Log_info) then {
	diag_log format ["Position ShoreLine_Shore Found %1",GF_APZ_Safe_distance_Check_Ok];
	};		
	};
	}else{
