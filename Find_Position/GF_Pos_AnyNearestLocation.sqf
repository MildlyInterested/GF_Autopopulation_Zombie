...
GF_APZ_randomPos_AnyNearestLocation = {
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	while {true} do {GF_APZ_Safe_distance_Check_initialized};
	
	if (GF_APZ_Systemchat_info) then {
	systemchat format ["Position AnyNearestLocation Found %1",GF_APZ_Safe_distance_Check_Ok];
	};	

	if (GF_APZ_Diag_Log_info) then {
	diag_log format ["Position AnyNearestLocation Found %1",GF_APZ_Safe_distance_Check_Ok];
	};		
...
while {true} do  {uisleep 1;GF_APZ_Safe_distance_Check_Ok or _a isEqualTo GF_APZ_Find_Pos_Attempts};

if (GF_APZ_Safe_distance_Check_Ok isEqualTo false) then {
_Pos = _randomPos;
