...
[] spawn {
	while {true} do {
		...
		1.while {true} do {uisleep GF_APZ_Spawn_Loop; GF_APZ_Enemy_Side countSide allUnits < GF_APZ_Enemy_Max_Number};
		2.while {true} do {uisleep GF_APZ_Spawn_Loop; count allUnits < GF_APZ_Enemy_Max_Number};
		*/
		
		
		while {true} do {uisleep GF_APZ_Spawn_Loop; count allUnits < GF_APZ_Enemy_Max_Number};
		
		GF_APZ_Pos_initialized = false;
		
		//______________________ Locations List  ___________________
		
		[] spawn GF_APZ_Spawn_Locations;
		
		while {true} do {uisleep 1;GF_APZ_Pos_initialized};
